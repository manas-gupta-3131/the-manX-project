...
with app.app_context():
    ...
    User.query.filter(User.email != 'admin').delete()
    Team.query.delete()
    db.session.commit()
    ...
    ph1 = task(p1.id, 'Phase 1: Concept & Feasibility', '1', 'phase',
               0, 21, 100, 'completed', team=t_hw, order=0)
    t1_1 = task(p1.id, 'Market & regulatory analysis', '1.1', 'task',
                0, 7, 100, 'completed', pm1, None, ph1.id, order=1)
    t1_2 = task(p1.id, 'BOM cost estimation', '1.2', 'task',
                7, 7, 100, 'completed', l_hw, t_hw, ph1.id, order=2)
    t1_3 = task(p1.id, 'Feasibility sign-off', '1.3', 'milestone',
                14, 1, 100, 'completed', admin, None, ph1.id, milestone=True, order=3)
    t1_4 = task(p1.id, 'Architecture freeze', '1.4', 'task',
                14, 7, 100, 'completed', l_fw, t_fw, ph1.id, order=4)

    # Phase 2 — Design
    ph2 = task(p1.id, 'Phase 2: Hardware & Firmware Design', '2', 'phase',
               21, 63, 80, 'in_progress', team=t_hw, order=5)
    t2_1 = task(p1.id, 'Schematic design — metering AFE', '2.1', 'task',
                21, 21, 100, 'completed', l_hw, t_hw, ph2.id, critical=True, order=6)
    t2_2 = task(p1.id, 'Schematic design — 4G modem board', '2.2', 'task',
                21, 21, 100, 'completed', m1, t_hw, ph2.id, order=7)
    t2_3 = task(p1.id, 'PCB layout — layer stack review', '2.3', 'task',
                42, 14, 90, 'in_progress', m2, t_hw, ph2.id, critical=True, order=8)
    t2_4 = task(p1.id, 'Firmware — bootloader & HAL', '2.4', 'task',
                21, 28, 100, 'completed', l_fw, t_fw, ph2.id, order=9)
    t2_5 = task(p1.id, 'Firmware — DLMS/COSEM stack integration', '2.5', 'task',
                49, 28, 60, 'in_progress', m3, t_fw, ph2.id, critical=True, order=10)
    t2_6 = task(p1.id, 'Firmware — 4G data push', '2.6', 'task',
                56, 21, 40, 'in_progress', m4, t_fw, ph2.id, order=11)
    t2_7 = task(p1.id, 'Design review gate', '2.7', 'milestone',
                84, 1, 0, 'not_started', admin, None, ph2.id, milestone=True, order=12)

    # Phase 3 — Prototype
    ph3 = task(p1.id, 'Phase 3: Prototype Build & EVT', '3', 'phase',
               84, 42, 0, 'not_started', team=t_qa, order=13)
    t3_1 = task(p1.id, 'Gerber release & PCB fabrication', '3.1', 'task',
                84, 10, 0, 'not_started', l_hw, t_hw, ph3.id, critical=True, order=14)
    t3_2 = task(p1.id, 'Component procurement', '3.2', 'task',
                84, 14, 0, 'not_started', m7, t_mfg, ph3.id, order=15)
    t3_3 = task(p1.id, 'PCB assembly & bringup', '3.3', 'task',
                94, 7, 0, 'not_started', l_hw, t_hw, ph3.id, order=16)
    t3_4 = task(p1.id, 'EVT functional testing', '3.4', 'task',
                101, 14, 0, 'not_started', l_qa, t_qa, ph3.id, critical=True, order=17)
    t3_5 = task(p1.id, 'EVT sign-off milestone', '3.5', 'milestone',
                125, 1, 0, 'not_started', admin, None, ph3.id, milestone=True, order=18)

    # Phase 4 — DVT & Certification
    ph4 = task(p1.id, 'Phase 4: DVT & Certification', '4', 'phase',
               126, 42, 0, 'not_started', team=t_qa, order=19)
    t4_1 = task(p1.id, 'EMC pre-compliance testing', '4.1', 'task',
                126, 14, 0, 'not_started', m6, t_qa, ph4.id, order=20)
    t4_2 = task(p1.id, 'BIS/MID certification submission', '4.2', 'task',
                140, 21, 0, 'not_started', pm1, None, ph4.id, critical=True, order=21)
    t4_3 = task(p1.id, 'Field trial — 50 units', '4.3', 'task',
                140, 21, 0, 'not_started', l_qa, t_qa, ph4.id, order=22)
    t4_4 = task(p1.id, 'DVT sign-off & mass production go', '4.4', 'milestone',
                167, 1, 0, 'not_started', admin, None, ph4.id, milestone=True, order=23)
    ...
    g2_3  = task(p2.id, 'Zigbee FW — pairing & device registry', '2.3', 'task', 42, 21, 70, 'in_progress', l_fw, t_fw, g_ph2.id, critical=True, order=7)
    ...
    for pred, succ in deps_p2:
        db.session.add(Dependency(predecessor_id=pred.id, successor_id=succ.id, dep_type='FS'))

    # ═══════════════════════════════════════════════════════════════════════
    # PROJECT 3 — Voltage Stabilizer Mk-V (planning)
    # ═══════════════════════════════════════════════════════════════════════
    ...
    for pred, succ in deps_p3:
        db.session.add(Dependency(predecessor_id=pred.id, successor_id=succ.id, dep_type='FS'))

    # ═══════════════════════════════════════════════════════════════════════
    # PROJECT 4 — Solar Charge Controller (on hold)
    # ═══════════════════════════════════════════════════════════════════════
    ...
    for user, task_obj, pct in allocs:
        a = Allocation(user_id=user.id, task_id=task_obj.id, percent=pct,
    ...
    approvals_data = [
        # completed
        (t1_3, admin,  'Feasibility Gate',  'approved',  d(14)),
        (t2_7, pm1,    'Design Review',     'pending',   d(84)),
        (t2_7, admin,  'VP Sign-off',       'pending',   d(86)),
        (t3_5, pm1,    'EVT Gate',          'pending',   d(126)),
        (t3_5, admin,  'VP EVT Sign-off',   'pending',   d(128)),
        (t4_4, admin,  'Mass Production Go','pending',   d(168)),
        (g1_3, pm2,    'SoC Selection',     'approved',  d(21)),
        (g3_4, admin,  'Beta Launch Gate',  'pending',   d(119)),
        (s1_3, pm1,    'Concept Approval',  'pending',   d(51)),
        (sc1_3, pm2,   'Algorithm Freeze',  'approved',  d(-36)),
    ]
    for task_obj, approver, stage, status, due in approvals_data:
        ap = Approval(
    ...
    for actor, etype, eid, field, old, new in audit_entries:
